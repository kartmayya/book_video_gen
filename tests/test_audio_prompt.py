...
class TestBuildFilterCmd:
    def test_vocal_only(self):
        ...
        pipe_inputs = [cmd[j+1] for j, a in enumerate(cmd) if a == "-i" and cmd[j+1].startswith("pipe:")]
...
def test_audio_prompt_request_schema():
    ...
    req = AudioPromptRequest(audio_prompt="test", book_id="b1", gap_between_lines_ms=500)
